# README – Result files imbalanced dataset

## Overview

This folder contains 8,335 `.RDS` files. Each file is a serialized R data frame representing the performance evaluation of one artificial distirbution map with several evaluation metrics. The evaluation metrics were calculated on an imbalanced dataset.
...
The evaluation metrics were calculated using the script `R/05_calculation_evaluation_metrics.R` The evaluation metrics were calculated on an imbalanced dataset, meaning 10 times the number of background/artificial-absence points than presence points were used to calcualte evaluation metrics. This script compares the artificial distribution maps in the `/data/resultsMain/maps` folder against the virtual species' true occurrence probability.
