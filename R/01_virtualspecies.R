...
#'@description 
#' This script generates 10 virtual species distributions across Southeast Australia 
#' (NSW, VIC, ACT) using WorldClim bioclimatic variables. The species parameters and niche 
#' definitions are derived from the study by **Grimmett et al. (2020)**. It performs three main tasks:
#' 1. Environmental data preprocessing (cropping, masking, and projecting to EPSG:3577).
#' 2. Generation of suitability rasters based on normal distribution functions (dnorm).
#' 3. Conversion to Presence-Absence data and replicated spatial sampling.
#' @references 
#' Grimmett, et al. (2020). "Presence-only species distribution models are sensitive to sample prevalence: 
#' Evaluating models using spatial prediction stability and accuracy metrics." Ecological Modelling. 
...
# create VS01 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS01.RDS"))){
  ...
  saveRDS(VS01, paste0("data/virtualSpecies/", "VS01.RDS"));rm(VS01,paramsVS01)
}

# create VS02 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS02.RDS"))){
  ...
  saveRDS(VS02, paste0("data/virtualSpecies/", "VS02.RDS"));rm(VS02,paramsVS02)
}
# create VS03 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS03.RDS"))){
  ...
  saveRDS(VS03, paste0("data/virtualSpecies/", "VS03.RDS"));rm(VS03,paramsVS03)
}

# create VS04 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS04.RDS"))){
  ...
  saveRDS(VS04, paste0("data/virtualSpecies/", "VS04.RDS"));rm(VS04,paramsVS04)
}

# create VS05 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS05.RDS"))){
  ...
  saveRDS(VS05, paste0("data/virtualSpecies/", "VS05.RDS"));rm(VS05,paramsVS05)
}

# create VS06 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS06.RDS"))){
  ...
  saveRDS(VS06, paste0("data/virtualSpecies/", "VS06.RDS"));rm(VS06,paramsVS06)
}

# create VS07 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS07.RDS"))){
  ...
  saveRDS(VS07, paste0("data/virtualSpecies/", "VS07.RDS"));rm(VS07,paramsVS07)
}

# create VS08 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS08.RDS"))){
  ...
  saveRDS(VS08, paste0("data/virtualSpecies/", "VS08.RDS"));rm(VS08,paramsVS08)
}

# create VS09 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS09.RDS"))){
  ...
  saveRDS(VS09, paste0("data/virtualSpecies/", "VS09.RDS"));rm(VS09,paramsVS09)
}


# create VS10 ####
#----------------#
if(!file.exists(paste0("data/virtualSpecies/", "VS10.RDS"))){
  ...
  saveRDS(VS10, paste0("data/virtualSpecies/", "VS10.RDS"));rm(VS10,paramsVS10)
}

# ================================================================ #
# 4. Conversion to PA data ----
# ================================================================ #
...
# prevalence values from Grimmet et al. 2020
...
for(v in 1:length(vs)){
  ...
  if(!file.exists(paste0("data/paRaster/", strsplit(vs[v], "/")[[1]][3]))){
    ...
    prob = terra::unwrap(PA$probability.of.occurrence)
    terra::writeRaster(PA, paste0("data/paRaster/", strsplit(vs[v], "/")[[1]][3]))
    terra::writeRaster(prob,  paste0("data/virtualSpecies/", strsplit(vs[v], "/")[[1]][3])) # save probability of occurrence
    #clean up
    rm(PA, species, prob)
  }
}
...
# ================================================================ #
# 5. Sample presence absence data ----
# ================================================================ #
...
for (i in 1:nrow(grid)){
  if(!file.exists( paste0("data/PA/",grid$vs[i],"_",grid$points[i],"_",grid$replicates[i],".gpkg"))){
